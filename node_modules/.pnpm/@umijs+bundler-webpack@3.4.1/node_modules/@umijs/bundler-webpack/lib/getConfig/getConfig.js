...
function _getConfig() {
  _getConfig = _asyncToGenerator(function* (opts) {
    ...
    ...(process.env.APP_ROOT ? [process.cwd()] : [])]).end().exclude.add(/node_modules/).end().use('babel-loader').loader(require.resolve('@umijs/deps/compiled/babel-loader')).options(babelOpts);
      ...
      config.extraBabelIncludes.forEach((include, index) => {
        ...
        webpackConfig.module.rule(rule).test(/\.(js|mjs|jsx)$/).include.add(a => {
          ...
          if ((0, _path().isAbsolute)(include)) {
            return (0, _path().isAbsolute)(include);
    ...
    if (process.env.UMI_DIR) {
      // prettier-ignore
      webpackConfig.module.rule('js-for-umi-dist').test(/\.(js|mjs|jsx)$/).include.add((0, _path().join)(process.env.UMI_DIR, 'dist', 'index.esm.js')).end().use('babel-loader').loader(require.resolve('@umijs/deps/compiled/babel-loader')).options(babelOpts);
    } // prettier-ignore
    ...
    if (process.env.PROGRESS !== 'none') {
      webpackConfig.plugin('progress').use(require.resolve('@umijs/deps/compiled/webpackbar'), [config.ssr ? {
