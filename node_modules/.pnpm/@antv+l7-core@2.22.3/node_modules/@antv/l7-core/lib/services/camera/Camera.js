...
let CAMERA_TYPE = exports.CAMERA_TYPE = /*#__PURE__*/function (CAMERA_TYPE) {
  CAMERA_TYPE["ORBITING"] = "ORBITING";
  CAMERA_TYPE["EXPLORING"] = "EXPLORING";
  CAMERA_TYPE["TRACKING"] = "TRACKING";
  return CAMERA_TYPE;
}({});
let CAMERA_TRACKING_MODE = exports.CAMERA_TRACKING_MODE = /*#__PURE__*/function (CAMERA_TRACKING_MODE) {
...
class Camera {
  ...
  setPerspective(near, far, angle, aspect) {
    this.fov = angle;
    this.near = near;
    this.far = far;
    this.aspect = aspect;
   ...
   */
  setFocalPoint(x, y, z) {
    let up = _glMatrix.vec3.fromValues(0, 1, 0);
    this.focalPoint = (0, _math.createVec3)(x, y, z);
    if (this.trackingMode === CAMERA_TRACKING_MODE.CINEMATIC) {
      const d = _glMatrix.vec3.subtract(_glMatrix.vec3.create(), this.focalPoint, this.position);
      x = d[0];
      y = d[1];
      z = d[2];
      const r = _glMatrix.vec3.length(d);
  ...
  setDistance(d) {
    ...
    const pos = _glMatrix.vec3.create();
  ...
  rotate(azimuth, elevation, roll) {
    if (this.type === CAMERA_TYPE.EXPLORING) {
      ...
      const rotX = _glMatrix.quat.setAxisAngle(_glMatrix.quat.create(), [1, 0, 0], (this.rotateWorld ? 1 : -1) * elevation * DEG_2_RAD);
      const rotY = _glMatrix.quat.setAxisAngle(_glMatrix.quat.create(), [0, 1, 0], (this.rotateWorld ? 1 : -1) * azimuth * DEG_2_RAD);
      const rotZ = _glMatrix.quat.setAxisAngle(_glMatrix.quat.create(), [0, 0, 1], roll * DEG_2_RAD);
      let rotQ = _glMatrix.quat.multiply(_glMatrix.quat.create(), rotY, rotX);
      rotQ = _glMatrix.quat.multiply(_glMatrix.quat.create(), rotQ, rotZ);
      const rotMatrix = _glMatrix.mat4.fromQuat(_glMatrix.mat4.create(), rotQ);
   ...
   */
  pan(tx, ty) {
    const coords = (0, _math.createVec3)(tx, ty, 0);
    const pos = _glMatrix.vec3.clone(this.position);
  ...
  computeMatrix() {
    ...
    const rotZ = _glMatrix.quat.setAxisAngle(_glMatrix.quat.create(), [0, 0, 1], this.roll * DEG_2_RAD);
    _glMatrix.mat4.identity(this.matrix);

    // only consider HCS for EXPLORING and ORBITING cameras
    const rotX = _glMatrix.quat.setAxisAngle(_glMatrix.quat.create(), [1, 0, 0], (this.rotateWorld || this.type === CAMERA_TYPE.TRACKING ? 1 : -1) * this.elevation * DEG_2_RAD);
    const rotY = _glMatrix.quat.setAxisAngle(_glMatrix.quat.create(), [0, 1, 0], (this.rotateWorld ? 1 : -1) * this.azimuth * DEG_2_RAD);
    let rotQ = _glMatrix.quat.multiply(_glMatrix.quat.create(), rotY, rotX);
    rotQ = _glMatrix.quat.multiply(_glMatrix.quat.create(), rotQ, rotZ);
    const rotMatrix = _glMatrix.mat4.fromQuat(_glMatrix.mat4.create(), rotQ);
    if (this.type === CAMERA_TYPE.ORBITING || this.type === CAMERA_TYPE.EXPLORING) {
  ...
  _getAngles() {
    ...
    const r = _glMatrix.vec3.length(this.distanceVector);
