...
export type ScaleTypeName = 'linear' | 'power' | 'log' | 'identity' | 'time' | 'quantile' | 'quantize' | 'threshold' | 'diverging' | 'sequential' | 'cat' | 'custom';
...
export interface IVertexAttributeDescriptor extends Omit<IAttributeInitializationOptions, 'buffer'> {
    ...
    update?: (feature: IEncodeFeature, featureIdx: number, vertex: number[], attributeIdx: number, normal: number[], vertexIndex?: number) => number[];
}
...
export type Triangulation = (feature: IEncodeFeature, styleOption?: unknown) => {
    vertices: number[];
    indices: number[];
    size: number;
    normals?: number[];
...
export interface IStyleAttributeService {
    ...
    createAttributesAndIndices(encodedFeatures: IEncodeFeature[], triangulation?: Triangulation, styleOption?: unknown, layer?: ILayer): {
        attributes: {
            [attributeName: string]: IAttribute;
        };
        elements: IElements;
     ...
     */
    updateAttributeByFeatureRange(attributeName: string, features: IEncodeFeature[], startFeatureIdx?: number, endFeatureIdx?: number, layer?: ILayer): void;
    /**
     * 清除当前管理的所有属性
     */
