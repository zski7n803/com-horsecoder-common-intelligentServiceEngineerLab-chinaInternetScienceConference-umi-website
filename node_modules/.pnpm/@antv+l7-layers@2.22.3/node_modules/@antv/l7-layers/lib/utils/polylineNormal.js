...
function computeMiter(tangent, miter, lineA, lineB, halfThick) {
  ...
  const tmp = _glMatrix.vec2.fromValues(-lineA[1], lineA[0]);
...
function _default(points, closed, indexOffset, isDash = true) {
  const lineA = _glMatrix.vec2.fromValues(0, 0);
  const lineB = _glMatrix.vec2.fromValues(0, 0);
  const tangent = _glMatrix.vec2.fromValues(0, 0);
  const miter = _glMatrix.vec2.create();
  let started = false;
  let lineNormal = null;
  ...
  for (let i = 1; i < total; i++) {
    const index = count;
    const last = _glMatrix.vec2.fromValues(points[i - 1][0], points[i - 1][1]);
    const cur = _glMatrix.vec2.fromValues(points[i][0], points[i][1]);
    let next = i < points.length - 1 ? _glMatrix.vec2.fromValues(points[i + 1][0], points[i + 1][1]) : null;
    // 如果当前点和前一点相同，跳过
    ...
    if (!next) {
      ...
      let miterLen = computeMiter(tangent, _glMatrix.vec2.fromValues(miter[0], miter[1]), lineA, lineB, 1);

      // get orientation
      const flip = _glMatrix.vec2.dot(tangent, lineNormal) < 0 ? -1 : 1;
  ...
  for (let i = 0; i < miters.length; i++) {
    ...
    pickData.push(attrPos[i * 3], attrPos[i * 3 + 1], attrPos[i * 3 + 2], attrDistance[i],
    // dash
    miters[i], totalDistance // dash
    );
  }
  return {
    normals: out,
