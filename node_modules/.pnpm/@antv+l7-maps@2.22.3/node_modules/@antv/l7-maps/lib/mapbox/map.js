...
const _excluded = ["id", "attributionControl", "style", "token", "rotation", "mapInstance"];
/**
 * MapboxService
 */
...
class MapboxService extends _BaseMapService.default {
  ...
  init() {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      if (token === MAPBOX_API_KEY && style !== 'blank' && !window.mapboxgl.accessToken && !mapInstance // 如果用户传递了 mapInstance，应该不去干预实例的 accessToken。
      ) {
      ...
      if (mapInstance) {
        ...
        // @ts-ignore
        _this.map = new window.mapboxgl.Map((0, _objectSpread2.default)({
  ...
  meterToCoord(center, outer) {
    ...
    const centerLnglat = new _mapboxGl.default.LngLat(center[0], center[1]);
    const outerLnglat = new _mapboxGl.default.LngLat(outer[0], outer[1]);
    const meterDis = centerLnglat.distanceTo(outerLnglat);

    // Tip: 三维世界坐标距离

    const centerMercator = _mapboxGl.default.MercatorCoordinate.fromLngLat({
      lng: center[0],
      lat: center[1]
    });
    const outerMercator = _mapboxGl.default.MercatorCoordinate.fromLngLat({
      lng: outer[0],
      lat: outer[1]
    });
    const {
